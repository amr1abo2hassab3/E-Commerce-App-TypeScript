...
>("whishList/removeItemFromWishList", async (userData: CartValues, thunkApi) => {
  const { rejectWithValue } = thunkApi;
  try {
    const request =  axiosInstance.delete<addWishListResponse>(
      `/api/v1/wishlist/${userData.productId}`,
      {
        headers: {
